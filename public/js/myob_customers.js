function getMyobCustomerDets(custUID) {
    ...
    $.ajax({
        type: "get",
        async: false,
        url: "http://localhost/~allanhyde/authentication/public/myob/customers/get-cust/get?custUID=" + custUID,
...
function updateCustomerDets(mode, updateCompanyName, updatedJson, updateMode, updateUID, id, thisUpdate, numUpdates, updateType){
    ...
    $.ajax({
        type: "POST",
        //async: false,
        url: "http://localhost/~allanhyde/authentication/public/myob/customers/update-cust/update?type=" + updateType + "&mode=" + updateMode + "&payload=" + updatedJson,
        success: function (data) {
            if (mode == 'invoice') {
                console.log("INV: " + data);
                if(data == 'update_errors') {
                    window.location.replace('http://localhost/~allanhyde/authentication/public/events/invoice');
                ...
                if(thisUpdate == numUpdates) {
                    ...
                    if(updateType == 'M') {
                        ...
                        if (getTradingName.length > 0) {
                            window.location.replace('http://localhost/~allanhyde/authentication/public/members/name/' + getTradingName + '');
                        } else {
                            window.location.replace('http://localhost/~allanhyde/authentication/public/members/list/' + membersFilter + '');
                            //console.log("ok here")
                        }
                        ...
                        if (getTradingName.length > 0) {
                            ...
                            window.location.replace('http://localhost/~allanhyde/authentication/public/stakeholders/list/' + membersFilter + '');
...
function getCustomerDets(mode, id, thisUpdate, numUpdates, updateType) {
    ...
    if(updateType == 'M') {
        var memberUpdateUrl = "http://localhost/~allanhyde/authentication/public/members/update/" + id;
    } else if(updateType == 'S') {
        var memberUpdateUrl = "http://localhost/~allanhyde/authentication/public/stakeholder/update/" + id;
    }
...
function updateCustomer(mode, memberId, updateType) {
    ...
    if(updateType == 'M') {
        var getCustomerUpdatesUrl = "http://localhost/~allanhyde/authentication/public/members/updates";
    } else if (updateType == 'S'){
        var getCustomerUpdatesUrl = "http://localhost/~allanhyde/authentication/public/stakeholders/updates";
    }
